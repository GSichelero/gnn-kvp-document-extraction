...
Complete pipeline for Brazilian energy bill (fatura de energia) processing:
...
class KVPExtractor:
    ...
    def _extract_rule_based(self, text_blocks: List[TextBlock]) -> List[KeyValuePair]:
        """Rule-based extraction for Brazilian energy bills"""
        pairs = []
        
        # Common patterns in Brazilian energy bills
        patterns = {
            "Cod Cliente": r"(?:C[óo]d(?:igo)?\.?\s*Cliente|Cliente)\s*[:\-]?\s*(\d+)",
            "Conta Contrato": r"Conta\s*Contrato\s*[:\-]?\s*(\d+)",
            "Instalação": r"Instala[çc][ãa]o\s*[:\-]?\s*(\d+)",
            "Vencimento": r"Vencimento\s*[:\-]?\s*(\d{2}[\/\-]\d{2}[\/\-]\d{2,4})",
            "Total": r"(?:Total|Valor)\s*(?:a\s*Pagar|Fatura)?\s*[:\-]?\s*R?\$?\s*([\d\.,]+)",
            "Mês Referência": r"(?:M[êe]s\s*Ref(?:er[êe]ncia)?|Refer[êe]ncia)\s*[:\-]?\s*(\w+[\/\-]?\d*)",
            "CNPJ": r"CNPJ\s*[:\-]?\s*([\d\.\/\-]+)",
            "Nota Fiscal": r"(?:N[úu]mero\s*)?(?:Nota\s*Fiscal|NF)\s*[:\-]?\s*(\d+)",
        }
...
class FieldClassifier:
    ...
    def _classify_rule_based(self, pairs: List[KeyValuePair]) -> Dict[str, List[KeyValuePair]]:
        """Simple rule-based classification"""
        category_keywords = {
            "customer": ["cliente", "consumidor", "titular", "nome"],
            "account": ["conta", "contrato", "instalação", "instalacao"],
            "billing": ["total", "valor", "fatura", "pagar"],
            "dates": ["vencimento", "emissão", "leitura", "referência"],
            "meter": ["medidor", "leitura", "kwh", "consumo"],
            "address": ["endereço", "rua", "cidade", "cep", "estado"],
            "fiscal": ["nota", "cnpj", "série", "fiscal"],
        }
...
Examples:
    ...
    python unified_document_pipeline.py --image fatura.png
    
    # Process with custom output directory
    python unified_document_pipeline.py --image fatura.png --output results/
    
    # Use Tesseract instead of EasyOCR
    python unified_document_pipeline.py --image fatura.png --ocr tesseract
    
    # Process multiple images
    python unified_document_pipeline.py --images_dir faturas-images/ --output results/
        """
    )
