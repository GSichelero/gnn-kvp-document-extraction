...
Tests all 5 stages of the optional unified document pipeline on local images
...
def print_results_summary(result_data: Dict, image_path: str, processing_time: float):
    ...
    for i, t in enumerate(tables):
        ...
        print(f"   Table {i+1}: {rows} rows x {cols} columns, {hdrs} headers")
    ...
    print(f"\nCategories: {len(categories)} identified")
...
Examples:
    ...
    python test_full_pipeline.py --image path/to/document.jpg
...
"""
    ...
    parser.add_argument(
        "--images_dir",
        type=str,
        default=os.environ.get("KVP_IMAGES_DIR", "data/images"),
        help="Images directory for batch",
    )
