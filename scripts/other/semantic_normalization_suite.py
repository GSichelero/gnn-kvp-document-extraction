    ...
    def analyze_normalization_needs(self) -> Dict:
        ...
        for doc in self.data:
            ...
            for pair in kvp_pairs:
                ...
                # Classify and collect normalization needs from value format.
                if re.search(r'\b\d{1,2}[/-]\d{1,2}[/-]\d{2,4}\b', value_text):
                    analysis['date_values'][value_text] += 1
                    analysis['normalization_cases']['dates'].append(value_text)

                elif re.search(r'R?\$?\s*\d+[.,]\d{2}\b', value_text):
...
class SemanticNormalizer:
    ...
    def normalize_amount(self, amount_str: str) -> Optional[float]:
        ...
        if '.' in amount_str and ',' in amount_str:
            ...
            # Only comma - likely decimal-comma style
    ...
    def _test_normalization_accuracy(self,
        ...
        for data_type, cases in test_cases.items():
            ...
            for input_val, expected in cases:
                ...
                logger.info(
                    f"  {input_val} -> {result} "
                    f"(expected {expected}) {'ok' if is_correct else 'fail'}"
                )
...
def main():
    ...
    annotation_dir = Path(os.environ.get("KVP_ANNOTATION_DIR", "data/annotations"))
    output_dir = os.environ.get("SEMANTIC_NORMALIZATION_OUTPUT_DIR", "semantic_normalization_results")
