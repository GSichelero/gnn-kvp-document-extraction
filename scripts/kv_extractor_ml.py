#!/usr/bin/env python3
"""
ML-Enhanced Key-Value Pair Extractor for Brazilian Electric Utility Invoices
Uses Graph Neural Network to learn entity relationships from ground truth annotations
"""
...
class TextFeatureExtractor:
    ...
    # Keywords indicating keys
    KEY_KEYWORDS = {
        'data', 'valor', 'total', 'subtotal', 'número', 'nº', 'código', 'cnpj', 'cpf',
        'endereço', 'telefone', 'fone', 'classe', 'atividade', 'período', 'vencimento',
        'competência', 'série', 'cfop', 'inscrição', 'tensão', 'medidor', 'transformador',
        'consumo', 'demanda', 'tarifa', 'quantidade', 'qtde', 'referência', 'apresentação',
        'emissão', 'cliente', 'conta', 'rota', 'fator', 'limite', 'mês', 'alíquota',
        'base', 'cálculo', 'pagar', 'fatura', 'tusd', 'icms', 'pis', 'cofins'
    }
    ...
    def build_vocab(self, texts: List[str]):
        ...
        # Add words that appear at least once
    ...
    def extract_text_features(self, entity: TextEntity) -> np.ndarray:
        ...
        keyword_match = sum(1 for kw in self.KEY_KEYWORDS if kw in text_lower)
        features.append(min(keyword_match / 3.0, 1.0))  # Keyword ratio
...
class MLKVExtractor:
    ...
    def prepare_training_data(self, annotations: List[dict]) -> List[dict]:
        ...
        self.feature_extractor.build_vocab(all_texts)
    ...
    def _heuristic_pairs(self, entities: List[TextEntity]) -> List[KVPair]:
        ...
        key_pattern = re.compile(r'.+:$|(?i)(data|valor|total|número|cnpj|classe|período).*')
        
        for key_entity in entities:
            if not key_pattern.match(key_entity.text.strip()):
                continue
            ...
            for value_entity in entities:
                ...
                # Check if to the right and aligned
                if value_entity.x1 > key_entity.x2:
                    y_overlap = min(key_entity.y2, value_entity.y2) - max(key_entity.y1, value_entity.y1)
                    if y_overlap > min(key_entity.height, value_entity.height) * 0.5:
...
def main():
    ...
    parser.add_argument("--annotation-dir", "-a", default="faturas-images/annotations")
